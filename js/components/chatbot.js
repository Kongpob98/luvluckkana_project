...
(function() {
    ...
    function isRelevantQuestion(userMessage) {
        ...
        const relevantKeywords = [
            ...
            // ความสัมพันธ์
            'แฟน', 'แฟนเก่า', 'คนเก่า', 'รีเทิร์น', 'คืนดี', 'กลับมาไหม', 'คู่แท้', 'เนื้อคู่', 'คู่ชีวิต',
            // คำทั่วไป
    ...
    function detectIntentFromText(text = '') {
        ...
        if (/(ความรัก|รัก|เนื้อคู่|คู่แท้|คู่ชีวิต|คนคุย|แฟน|สถานะความสัมพันธ์|คนที่ใช่|soulmate)/i.test(text)) {
            return 'love';
        }
    ...
    function buildClarificationPlan(userMessage) {
        ...
        if (intent === 'love') {
            const loveQuestions = [];
            const asksSoulmate = /(คู่แท้|เนื้อคู่|คู่ชีวิต|คนที่ใช่|soulmate)/i.test(message);

            if (asksSoulmate) {
                loveQuestions.push({
                    key: 'loveFocus',
                    text: 'อยากโฟกัสเรื่องไหนคะ: ช่วงเวลาที่จะได้พบคู่แท้ หรือ ลักษณะนิสัยของคนที่จะเข้ามา'
                });
            }
